...
export const MMU = {
    ...
    init: function () {
        console.log('[MMU] init()');
        this.mem = new ArrayBuffer(1024 * 1024);
        this.vga = new Uint8Array(this.mem, 0xa000 << 4, 64 * 1024);
        this.mem8 = new Uint8Array(this.mem);
        this.mem16 = new DataView(this.mem);
    },
    ...
    rb: function (addr) {
        try {
            ...
            console.error('mem range error: ' + addr);
            return 0;
        }
    },

    mapUint8Array: function (buffer, addr, size) {
        const arr = new Uint8Array(this.mem, addr, size);
    ...
    rbs: function (seg, addr) {
        if (arguments.length < 2) throw 'rbs requires two arguments, called with only ' + arguments.length;
    ...
    wbs: function (seg, addr, value) {
        if (arguments.length < 3) throw 'wbs requires two arguments, called with only ' + arguments.length;
    ...
    rws: function (seg, addr) {
        if (arguments.length < 2) throw 'rws requires two arguments, called with only ' + arguments.length;
    ...
    wws: function (seg, addr, value, swap) {
        if (arguments.length < 3) throw 'wws requires two arguments, called with only ' + arguments.length;
    ...
    rss: function (seg, addr) {
        if (arguments.length < 2) throw 'rss requires two arguments, called with only ' + arguments.length;
